...
load_highschoolfriendship_network <- function() {
  zelfile <- gzfile('../data/Friendship-network_data_2013.csv.gz')
  el <- read.table(zelfile, header=F)
  el$V1 <- as.character(el$V1)
  el$V2 <- as.character(el$V2)
  g <- graph_from_edgelist(as.matrix(el))

  metadata <-read.table('../data/metadata_2013.txt', header=F,stringsAsFactors=F)
