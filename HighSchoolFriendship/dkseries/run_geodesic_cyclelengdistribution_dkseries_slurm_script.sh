...
#
# generate tables using  makeGeodesicCycleLEngthDistributionTables.sh
# (only run if not exist; so to regenerate remove these files first)
#

if [ ! -f $OBS_GEODESICCYCLETAB ]; then 
	time $SCRIPTDIR/makeSingleNetworkGeodesicCycleLengthDistributionTable.sh highschoolfriendship_edgelist.txt > ${OBS_GEODESICCYCLETAB}
fi
