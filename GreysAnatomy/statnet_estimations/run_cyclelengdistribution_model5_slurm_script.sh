...
#SBATCH --job-name="cypath_model5"
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=1
#SBATCH --cpus-per-task=1
#SBATCH --time=0-0:10:00
#SBATCH --output=cypath_model5-%j.out
#SBATCH --error=cypath_model5-%j.err
...
ATOMICTAB=model5_atomiccycledisttable.txt
CHORDLESSTAB=model5_chordlesscycledisttable.txt
CYCLETAB=model5_cycledisttable.txt
...
#
if [ ! -f $ATOMICTAB ]; then 
	time $SCRIPTDIR/makeAtomicCycleLengthDistributionTables.sh simulated/model5/ > ${ATOMICTAB}
fi

if [ ! -f $CHORDLESSTAB ]; then 
	time $SCRIPTDIR/makeCycleLengthDistributionTables.sh C simulated/model5/ > ${CHORDLESSTAB}
fi

if [ ! -f $CYCLETAB ]; then 
	time $SCRIPTDIR/makeCycleLengthDistributionTables.sh c simulated/model5 > ${CYCLETAB}
fi
