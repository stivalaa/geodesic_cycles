...
OBS_ATOMICCYCLETAB=zachary_karateclub_atomiccycletable.txt
OBS_CHORDLESSCYCLETAB=zachary_karateclub_chordlesscycletable.txt
...
if [ ! -f $OBS_ATOMICCYCLETAB ]; then 
	time $SCRIPTDIR/makeSingleNetworkAtomicCycleLengthDistributionTable.sh zachary_karateclub_edgelist.txt > ${OBS_ATOMICCYCLETAB}
fi

if [ ! -f $OBS_CHORDLESSCYCLETAB ]; then 
	time $SCRIPTDIR/makeSingleNetworkCycleLengthDistributionTable.sh C zachary_karateclub_edgelist.txt > ${OBS_CHORDLESSCYCLETAB}
fi
