...
#SBATCH --job-name="atmoic_chordless_cypath_model2"
#SBATCH --nodes=1
...
OBS_ATOMICCYCLETAB=kapferer_tailorshop_atomiccycletable.txt
OBS_CHORDLESSCYCLETAB=kapferer_tailorshop_chordlesscycletable.txt
...
if [ ! -f $OBS_ATOMICCYCLETAB ]; then 
	time $SCRIPTDIR/makeSingleNetworkAtomicCycleLengthDistributionTable.sh kapferer_tailorshop_edgelist.txt > ${OBS_ATOMICCYCLETAB}
fi

if [ ! -f $OBS_CHORDLESSCYCLETAB ]; then 
	time $SCRIPTDIR/makeSingleNetworkCycleLengthDistributionTable.sh C kapferer_tailorshop_edgelist.txt > ${OBS_CHORDLESSCYCLETAB}
fi
