...
OBS_GEODESICCYCLETAB=lazega_lawfirm_friendship_geodesiccycletable.txt
...
if [ ! -f $OBS_GEODESICCYCLETAB ]; then 
	time $SCRIPTDIR/makeSingleNetworkGeodesicCycleLengthDistributionTable.sh lazega_lawfirm_friendship_edgelist.txt > ${OBS_GEODESICCYCLETAB}
fi
