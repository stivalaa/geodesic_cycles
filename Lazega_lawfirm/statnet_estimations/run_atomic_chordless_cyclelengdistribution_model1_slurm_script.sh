...
#SBATCH --job-name="atomic_chordless_cypath_model1"
#SBATCH --nodes=1
...
OBS_CHORDLESSCYCLETAB=lazega_lawfirm_friendship_chordlesscycletable.txt
...
if [ ! -f $OBS_CHORDLESSCYCLETAB ]; then 
	time $SCRIPTDIR/makeSingleNetworkCycleLengthDistributionTable.sh C lazega_lawfirm_friendship_edgelist.txt > ${OBS_CHORDLESSCYCLETAB}
fi
