...
"""Usage: filterCyclesGeodesic.R graph_egelist.txt
   ...
   so for example you can get a histogram of geodesic cycle lengths with:
  ...
  Even using exactly the same algorithm and igraph library
  (i.e. precomputing distance matrix), Python is several times faster
  than R version even on small examples, and this vrsion, memoizing
  single-source shortest paths computation (instead of precomputing
  all-pairs shortest path) allows larger networks to be used, where
  precomputing the distance matrix is too slow and uses too much
  memory.
...
g = None #global variable for graph g otherwise cannot memoize geodesic

@functools.cache # Memoize the geoesic function (Python 3.9)
def geodesic(u):
    """
    ...
    Note although Graph and list are not hashable (so cannot be used
    as parameters in memoized function), we can return a list (return value
    does not need to be hashed). So instead of memoizing geodesic(u, v)
    we more efficiently memoize geodesic(u) which is the list of all geodesics
    from node u.
...
def isCycleGeodesic(cycle):
    ...
    # this needs to be made a list as it is used twice so would not work
    # the second time if kept as an iterator
    ...
    node_indices = dict([(u, cycle.index(u)) for u in cycle])
    nodepair_indices = [(node_indices[u], node_indices[v])
...
def main():
    ...
    sys.stderr.write("geodesic cache info: " +
                     str(geodesic.cache_info()) + '\n')
