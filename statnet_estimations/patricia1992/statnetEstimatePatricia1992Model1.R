#!/usr/bin/env Rscript
###
### File:    statnetEstimatePatricia1992Model1.R
### Author:  Alex Stivala
### Created: July 2019
###
...
###
### Usage: statnetEstimatePatricia1992Model1.R
###
###
### Model and GoF output is to stdout, also writes output files to cwd:
###   'patricia1992_statnet_model1.txt'
###   'patricia1992_statnet_model1_mcmcdiagnostics.eps'
###   'patricia1992_statnet_model1_gof.eps'
...
system.time( model1 <- ergm(gn ~ edges + gwdegree(0.5,fixed=TRUE) + gwesp(0, fixed=TRUE), control = control.ergm(main.method="MCMLE")) )

postscript('patricia1992_statnet_model1_mcmcdiagnostics.eps')
mcmc.diagnostics(model1)
dev.off()

sink('patricia1992_statnet_model1.txt')
...
print(model1_gof)
postscript('patricia1992_statnet_model1_gof.eps')
par(mfrow=c(3,3))
plot( model1_gof)
dev.off()
