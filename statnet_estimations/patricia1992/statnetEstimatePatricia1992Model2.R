#!/usr/bin/env Rscript
###
### File:    statnetEstimatePatricia1992Model2.R
### Author:  Alex Stivala
### Created: July 2019
###
### Estimating ERGM parameters for a model for Patricia 1992 delusional
### social network.x
###
### Citations for data:
###
...
###
### Usage: statnetEstimatePatricia1992Model2.R
###
###
### Model and GoF output is to stdout, also writes output files to cwd:
###   'patricia1992_network_drawing_statnet.eps'
###   'patricia1992_statnet_model2.txt'
###   'patricia1992_statnet_model2_mcmcdiagnostics.eps'
###   'patricia1992_statnet_model2_gof.eps'
...
g = read.graph('patricia1992.graphml', format='graphml')
attrs <- read.table('patricia1992_attributes.txt', stringsAsFactors= FALSE, header=TRUE)
...
postscript('patricia1992_network_drawing_statnet.eps')
...
postscript('patricia1992_statnet_model2_mcmcdiagnostics.eps')
mcmc.diagnostics(model2)
dev.off()

sink('patricia1992_statnet_model2.txt')
...
print(model2_gof)
postscript('patricia1992_statnet_model2_gof.eps')
par(mfrow=c(3,3))
plot( model2_gof)
dev.off()
